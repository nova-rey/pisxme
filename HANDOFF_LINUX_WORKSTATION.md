...
In this public recovery snapshot the bridge source paths are documented but
not present: the iCloud File Provider had not materialized `server.py`,
`bridge/`, or their test files when the snapshot was assembled. See
`RECOVERY_MANIFEST.md` for the exact omitted paths. Recover those files from a
fully materialized local/iCloud or Linux copy before relying on the bridge.
