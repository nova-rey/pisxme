...
- Bridge entry points in the original Mac worktree were `bridge/core.py`,
  `bridge/schematic_backend.py`, and `server.py`; those paths remain iCloud
  dataless and are explicitly omitted from this public recovery snapshot.
  Recover them before running bridge tests.
