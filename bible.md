...
2026-09-11 — Phase 24 regulator long-wire probe: removing the two remaining
65 mm regulator rail wires in a disposable copy replaced two endpoint-grid
findings with two label-dangling findings, leaving native ERC at 489 warnings
/ 0 errors while exact 338-net/node parity held. The wires are owned rail
connections; removal was rejected and canonical CAD was unchanged.
...
The three-corridor V2 storage support trial passes endpoint/negative-control
checks but creates source-field shorts/crossings and native DRC 644/409. It is
rejected route evidence; canonical copper remains unchanged.
...
2026-09-11 — Phase 24 second systemic ERC repair: removed 84 exact unowned
1 mm regulator pin-stub wires. Native ERC improved from 657 to 489 warnings/0
errors; native netlist remained exactly 338 nets with zero changed node sets,
and the hierarchy structure audit passed. Repair promoted as
source-authoritative; 201 endpoint-grid and 232 isolated-label findings
remain open and unwaived.
...
2026-09-11 — Phase 24 worker-baseline correction: qualified the isolated
KiCad Light ERC result as 539 total including 53 library-symbol and 3
footprint-link findings, unlike the live dirty 483/0 report. The reports are
not treated as like-for-like until library resolution is reconciled.
...
Support V2 also passed all ten saved-board USB3 connectivity assertions but
...
Support V4 passed all ten saved-board USB3 connectivity assertions but was
rejected by native DRC at 459/421 for U11 pad escape, RX return convergence,
and a support-channel crossing. The next repair remains local to U11/U12 and
does not reopen the accepted east source handoff or architecture.
...
Fresh EDA Light validation from committed ref `112e65be` reproduces the reset
primitive at 439/417 under KiCad 10.0.6; the count delta is retained as a tool
version difference.
...
Fresh EDA Light validation from committed ref `c8833d56` reproduces the VCCO
primitive at 432/419 under KiCad 10.0.6; the count delta is retained as a tool
version difference.
...
The corrected JMS_XAVDDH west-side escape from U11.52 to rehomed C84 passed
native saved-copper connectivity and its trace-removal negative control, but
fresh Light validation exposed a local solder-mask bridge at the QFN escape
(562/415). It is rejected route evidence; the REXT base remains active.
...
2026-09-12 — Duplicate-label probes preserved as raw evidence. Deletion caused
four native errors and lost 168 netlist nodes; local-label demotion preserved
...
## 2026-09-13 — Phase 24 third duplicate POWER_GND via producer

An isolated producer from `5ba33c28` removed one additional exact duplicate
`POWER_GND` via. Light DRC improved 310→309 with unconnected items unchanged
at 499 and no collateral family changes or shorts. Receipt and raw report are
retained; canonical integration remains pending.

## 2026-09-13 — Phase 24 fourth duplicate POWER_GND via producer

An isolated producer from `0ee5c0f4` removed one additional exact duplicate
`POWER_GND` via. Light DRC improved 309→308 with unconnected items unchanged
at 499 and no collateral family changes or shorts. Receipt and raw report are
retained; canonical integration remains pending.

## 2026-09-13 — Phase 24 fifth duplicate POWER_GND via producer

An isolated producer from `5b40f44f` removed one additional exact duplicate
