...
## 2026-09-04 native footprint-rotation correction

The rotated minimal clock graph was rerun with KiCad's native footprint
rotation convention corrected from `90` to `270` degrees, and the support
island was shifted west of the corrected SATA ancestor. Native KiCad DRC
measured `388` violations and `420` unconnected items. The prior local
clock-support short class was removed; the remaining new records are west
clock-trunk crossings against the inherited USB3 corridor plus inherited
zone/via-rule debt.

This candidate is rejected as an integration candidate, but the correction is
retained in the generic clock fixture authoring path. The proven SATA ancestor
and isolated clock fixture remain valid; Phase 19 is still open.
...
CM5 support field while retaining TX on B.Cu. Native KiCad DRC reported 212
records, four crossings, and three shorts. Failures were CM5 5 V support-pad
interference plus the SATA RX_P bypass junction. Rejected; layer allocation
alone is exhausted for the current U7/J3 placement.
...
relocation reference. Native KiCad DRC measured 320 records, including four
track crossings, five shorts, and 426 unconnected items. The baseline also
contains invalid via/zone interactions and CM5 source-escape failures. It is
rejected as an ancestor; a fresh parameterized U7/J3 island author is
required.
## 2026-09-05 SATA RX_P F.Cu coordinated trial

`PHASE19_COORDINATED_SATAP_F24.kicad_pcb` reassigned the SATA RX_P local
corridor to F.Cu to avoid USB B.Cu. Native KiCad DRC reported 215 records,
five crossings, one U7 NC-pad short, and an additional F.Cu SATA crossing.
Rejected; SATA270 remains the authoritative storage-side oracle.
