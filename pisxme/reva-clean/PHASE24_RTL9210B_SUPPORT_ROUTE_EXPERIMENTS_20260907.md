...
Native saved-board connectivity showed dangling 5V branches and DRC still
reported the pad-32 clearance violation. Reject V578; retain V576 as the best
connected candidate. The strict QFN edge escape remains open.
V577 edited the existing V576 U1.34 handoff natively by 0.05 mm. Native DRC
rose to 13 findings and still reported the strict pad-33/pad-34 source-field
clearance and pad-35 clearance. Reject V577; retain V576. The limiting
condition is the intrinsic 0.4 mm QFN edge field, not a missing saved-net
association.
...
at 9 findings with no new crossing. Retain V595 as the current disposable
PEDET/CLKREQ-plus-rail candidate, not production closure.
V596 is rejected because its proposed SPICS transition occupied the existing
...
V617-V619 reject SPICLK source/channel variants: source dogbone crossings
persisted against SPISI/RTL_5V even after moving the channel above the retained
field, and lower endpoint transitions entered 3V3. The next pass must
co-author the complete U1 QFN SPI source field.
...
V525 routes U1.39 from the existing outer 3V3 via around the lower/right
acreage to the C3-side handoff. Native U1.20/U1.34/U1.39 connectivity and
the exact branch-removal negative control pass, but native DRC finds 22
violations including crossings with RTL_5V and REFCLK_P. V525 is rejected as
...
pad cannot be added independently. Bottom 1V1 completion therefore requires
coherent source-field regeneration; Path A and production CAD remain
unchanged.
...
passing. This narrows the next task to regenerating the complete lower QFN
source field together; isolated U1.52 detours are no longer a valid route
class. No Path-A or production CAD changed.
V497 tested U1.52 RTL_3V3 to the retained rail. The route passed native
connectivity and its negative control, but the lower-edge escape crossed the
retained XTAL_IN via/escape and RSET source-field tracks. Native DRC rejected
the implementation for those clearance/crossing findings. V497 is retained
as a rejected route experiment; U1.52 remains open for co-allocation with the
completed support source field. No Path-A or production CAD changed.
...
one trace-removal negative control pass. Native DRC has zero errors; nine
inherited incomplete-fixture warnings remain. V494 is retained as the
RTL_5V source aggregation primitive; remaining supply pins and support nets
stay open. No Path-A or production CAD changed.
...
change was made.
## V555 outboard RTL_1V1 return — rejected

V555 routed the remaining U1.55/U1.60/U1.63 RTL_1V1 group on B.Cu to an
outboard x=112.4 mm rise and joined the valid upper RTL_1V1 trunk at y=57.3.
Native DRC still rejected the route: the rise crossed existing RTL_3V3 and
RTL_5V B.Cu trunks, while the pad escapes retained XTAL_OUT and RX/REFCLK
