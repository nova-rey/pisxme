...
The current committed Phase 24 checkpoint is `16bfc785`. Native KiCad 10.0.5
electrical ERC is 300 warnings / 0 errors: 121 `endpoint_off_grid`, 126
`isolated_pin_label`, 30 `same_local_global_label`, and 23
`multiple_net_names` (22 after the verified M2_GND alias normalization). Fresh
KiCad Light reports 355 total warnings / 0
...
For schematic-to-PCB parity, use the current native export
`PHASE24_CURRENT_NATIVE_NETLIST_20260912.xml`; the root-level
`PiSXMe_RevA_Clean.xml` is a SATA-only export from 2026-08-30 and is historical.
...
`PHASE24_FRESH_SOURCE_PCB_PARITY_RECEIPT_20260912.md`.
...
Two local JMS583 ground-return attempts were rejected for native shorts at
the U11 pad field; see
`PHASE24_JMS_LOCAL_GROUND_ESCAPE_REJECT_RECEIPT_20260912.md`. The U11
placement and architecture remain closed.
...
findings; the validation details are in
...
native KiCad test at 749 findings, including seven hierarchy pin-not-connected
errors. Canonical sources were untouched. Native-authored live-port hierarchy
regeneration remains required; full ERC and Phase 24 stay OPEN.
...
native DRC 0/0/0 plus the DFM audit. The first full-board support-placement
trial is rejected. The latest overlay reports 37 native DRC violations and 6
unconnected items; the preceding overlay reported 36 violations and 6 opens.
Both remain historical evidence. The next implementation is a fresh
...
V1583 tested an outer-board REFCLK overlay on the same fixed V1517 basis and
was rejected with 27 native DRC violations, including conflicts with the
existing lane and J1 launch. This is additional route-implementation
...
JMS_REXT focused audits also pass. Fresh native DRC is 603 / 342 versus the
...
interactions, and a GND crossing. V1138 then tried the consultant-recommended
separated y=75/y=77 return lanes but was rejected with 15 violations,
including XTAL_IN/XTAL_OUT return crossings and source-field conflicts. These
...
V1147 moved XTAL_IN left immediately after its lower transition and kept the
V1144 XTAL_OUT route unchanged. Its downstream/GND-avoidance corridor was
...
V774/V777 preserve electrically clean control-placement variants. V775/V776/
V778 are rejected PEDET/CLKREQ_N route implementations because they cross the
...
V679 direct crystal routing and V680 relocated-crystal routing were both
...
V696 was rejected for RTL_1V1 crossing and 3V3 clearance. V699's split
RTL_5V overpass now connects U1.33 to C5 with native DRC zero violations and
25 remaining opens. U1.17 RTL_5V remains open.
...
schematic: 485 warnings / 0 errors. The duplicate POWER_INPUT wire
serialization cluster is closed by
`PHASE24_POWER_DUPLICATE_WIRE_REPAIR_RECEIPT_20260911.md`. Remaining census:
197 `endpoint_off_grid`, 232
...
its source transitions intersected existing SPI corridors. V836's F.Cu route
reduced this to one crossing plus endpoint warnings. V837 reverses the
...
is still open.
V843 co-moved Y1/C1/C2/R1 away from the 90-degree rotated U2 field and passes
native DRC with 0 violations and 45 expected incomplete connections. V844
tested the 270-degree U2 orientation; it has no electrical DRC violations and
only 3 silkscreen warnings, with SPISI/SPICLK/SPISO3 ordered monotonically
...
crossing and remains historical evidence only. V926 moved the SPISO
layer-transition/final dogbone below the SPICS endpoint; native DRC is 0 with
25 expected incomplete items. V927 added SPISI using the same ordinary-via
escape discipline; native DRC is 0 with 24 expected incomplete items. The
saved-board V927 audit proves all five SPI nets and the U1.25 RTL_1V1-to-C4
...
outboard transition geometry around the accepted crystal/1V1 field. Native
DRC has 0 electrical violations with three inherited isolated-fill warnings
and 22 expected unrelated opens. The saved-board RTL_5V audit and corrected
C5-side trace-removal negative control pass. V940 is the accepted 1V1/RSET/
crystal/5V primitive; RTL_3V3 and local GND restoration remain open.
...
SPISI. The lower-drop class is therefore rejected as a route implementation;
...
V1013 is REJECTED: the U1.40 lower-left route contacted the vertical
RTL_3V3 collector at its actual via/track field. V1014 showed the same
collector conflict for the nominal U1.50 transition. V1015 moves that
transition west of the collector and passes native DRC with no electrical
...
V1027 corrected V1026's 0.2 mm replacement-GND endpoint mismatch to the
preserved (89.2,60.0) via, but native DRC still reports the same single
isolated GND-zone warning. The co-authored U1.40/RTL_3V3 field continues to
...
negative-control, and DRC evaluation.
V1313 is rejected. Although its saved-board four-pair endpoint audit and
negative controls pass, the pad-adjacent geometry produced 23 native DRC
violations, including source-field crossings/shorts, REFCLK interaction, and
