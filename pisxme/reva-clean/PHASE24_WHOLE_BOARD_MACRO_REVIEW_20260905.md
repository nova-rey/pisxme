...
## Current-basis reconciliation

The corrected basis is additionally measured in
`PHASE24_CURRENT_MACRO_TOPOLOGY_METRICS_20260905.md`, generated by native
KiCad loading of the selected candidate after the complete translated
Ethernet support island was integrated. This directly reports the selected
candidate rather than inferring it from the older baseline. It records
Ethernet same-net topology `97.45 mm` and complete storage USB3-source
same-net topology `145.11 mm`; these are placement/ratsnest metrics, not
routing-quality claims.
