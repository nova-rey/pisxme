...
|---|---|---|---|
| ERC grid/geometry | Isolated KiCad Light worker `erc-grid-20260911` plus Root Foreman integration | OPEN | Current KiCad 10.0.5 canonical ERC is 377/0: 197 `endpoint_off_grid` remain after the promoted label repair. The separate KiCad 10.0.6 worker result (539 total including library/link classes) is version-qualified evidence, not the current baseline. Next: map exact source owners before any grid transform. |
