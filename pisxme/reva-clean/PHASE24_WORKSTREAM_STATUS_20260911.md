...
| ERC labels/hierarchy | Isolated KiCad Light worker `erc-labels-20260911` plus Root Foreman integration | OPEN | Independent KiCad 10.0.6 worker reproduces the same 539 total and library/link delta; current KiCad 10.0.5 live-dirty census has 232 `isolated_pin_label` and 30 `same_local_global_label`. The census records 53 repeated warning coordinate/name signatures; live `STORAGE` source also has repeated connector-label records at identical coordinates. Next: test that bounded authoring hypothesis in a disposable copy. |
