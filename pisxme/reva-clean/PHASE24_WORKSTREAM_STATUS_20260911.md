...
|---|---|---|---|
| ERC grid/geometry | Isolated KiCad Light worker `erc-grid-20260911` plus Root Foreman integration | OPEN | Committed-ref native ERC reproduces 539 total (197 endpoint, 232 isolated-label, 30 same-local/global, 24 multi-name, plus 53 library and 3 footprint-link findings); current live-dirty census is 483/0 without library/link classes. Next: reconcile library resolution, then map exact owners before transform. |
| ERC labels/hierarchy | Isolated KiCad Light worker `erc-labels-20260911` plus Root Foreman integration | OPEN | Independent committed-ref worker reproduces the same 539 total and library/link delta; current live-dirty census has 232 `isolated_pin_label` and 30 `same_local_global_label`. Next: reconcile environment delta, then test one shared authoring pattern in a disposable copy. |
...
| DFM/native PCB | Isolated KiCad Light worker `phase24-dfm-20260911` plus Root Foreman integration | OPEN | Committed-ref native DRC is 180 violations / 468 unconnected; live dirty-tree check was 180/477. Both are candidate-scoped evidence, not closure. Reconcile source/copper delta before selecting the integrated target. |
