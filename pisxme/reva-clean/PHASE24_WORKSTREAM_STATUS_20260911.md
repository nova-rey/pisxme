...
|---|---|---|---|
| ERC grid/geometry | Isolated KiCad Light worker `erc-grid-20260911` plus Root Foreman integration | OPEN | Committed-ref native ERC reproduces 539/0; current live-dirty census is 483/0. 197 live `endpoint_off_grid` findings show root-sheet concentration and repeated x-coordinate families. Next: reconcile source delta, then map exact owners before transform. |
| ERC labels/hierarchy | Isolated KiCad Light worker `erc-labels-20260911` plus Root Foreman integration | OPEN | Committed-ref native ERC independently reproduces 539/0; current live-dirty census has 232 `isolated_pin_label` and 30 `same_local_global_label`. Next: reconcile source delta, then test one shared authoring pattern in a disposable copy. |
