...
def route(start,goal,occ,reserved):
 ...
 while q:
  ...
  for v in ((u0[0]+1,u0[1]),(u0[0]-1,u0[1]),(u0[0],u0[1]+1),(u0[0],u0[1]-1)):
   if not (88/STEP<=v[0]<=140/STEP and 116/STEP<=v[1]<=180/STEP): continue
...
layer_by_num={'52':pcbnew.B_Cu,'53':pcbnew.F_Cu,'54':pcbnew.B_Cu}
for num,name,xgoal in spec:
 ...
 goal=(xgoal,159.5)
 layer=layer_by_num[num]; occ=blocked(n.GetNetCode(),layer)
 start=s if layer==pcbnew.F_Cu else via
 path=route(start,goal,occ,reserved[layer]); print(num,len(path))
 ...
 for a,z in zip(path,path[1:]):
  t=pcbnew.PCB_TRACK(b); t.SetStart(V(*xy(a))); t.SetEnd(V(*xy(z))); t.SetLayer(layer); t.SetWidth(pcbnew.FromMM(.15)); t.SetNet(n); b.Add(t); reserved[layer].add(z)
