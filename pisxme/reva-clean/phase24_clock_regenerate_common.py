...
def blocked(netcode, layer):
    ...
    for t in b.GetTracks():
        if t.GetLayer()==layer and t.GetNetCode()!=netcode: mark(t.GetStart(),t.GetEnd())
...
for number, key in [('52','XI'),('54','XO'),('53','VS')]:
    ...
    for target in targets:
        ...
        path=route(current,target,blocked(net.GetNetCode(),layer),reserved[layer])
        for a,z in zip(path,path[1:]):
            t=pcbnew.PCB_TRACK(b); t.SetStart(V(*pxy(a))); t.SetEnd(V(*pxy(z))); t.SetLayer(layer); t.SetWidth(pcbnew.FromMM(.15)); t.SetNet(net); b.Add(t); reserved[layer].add(z)
        current=target
