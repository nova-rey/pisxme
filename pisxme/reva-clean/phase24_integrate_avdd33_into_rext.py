"""Rejoin the missing AVDD33 primitive onto the current REXT base."""
from pathlib import Path
import pcbnew
R=Path(__file__).resolve().parent
BASE=R/'PHASE24_STORAGE_MKEY_USB3_REXT_LOCAL_20260912.kicad_pcb'
OUT=R/'PHASE24_STORAGE_MKEY_USB3_REXT_AVDD33_LOCAL_20260912.kicad_pcb'
...
c.SetPosition(P(150,146))
for item in list(b.GetTracks()):
 if item.GetNetCode()==n.GetNetCode():b.RemoveNative(item)
s=u.FindPadByNumber('19').GetPosition();src=(pcbnew.ToMM(s.x),pcbnew.ToMM(s.y));dst=(pcbnew.ToMM(c.FindPadByNumber('1').GetPosition().x),pcbnew.ToMM(c.FindPadByNumber('1').GetPosition().y))
pts=[src,(142.2,139.5),(149.5,139.5),dst]
