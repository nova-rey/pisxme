"""Disposable native-pad JMS583 AVDD33 decoupler escape probe."""
from pathlib import Path
import pcbnew
R=Path(__file__).resolve().parent
BASE=R/'PHASE24_DUAL_MODE_STORAGE_PLACEMENT_CURRENT.kicad_pcb'
OUT=R/'PHASE24_JMS583_AVDD33_PROBE.kicad_pcb'
...
if not u or not c or not n: raise RuntimeError('missing AVDD33 objects')
c.SetPosition(P(148,143))
for x in list(b.GetTracks()):
    if x.GetNetCode()==n.GetNetCode(): b.RemoveNative(x)
s=u.FindPadByNumber('19').GetPosition();d=c.FindPadByNumber('1').GetPosition()
src=(pcbnew.ToMM(s.x),pcbnew.ToMM(s.y));dst=(pcbnew.ToMM(d.x),pcbnew.ToMM(d.y))
seg(b,n,src,(src[0],140));seg(b,n,(src[0],140),(147,140));seg(b,n,(147,140),(147,dst[1]));seg(b,n,(147,dst[1]),dst)
