...
for ref,x,y in [('R30',24,103),('R31',24,100)]:
...
for name,src,ref,via_xy,pts in [
 ('ETH_LEDY','17',( 'R30'),(0,0),[(32.96,101.9),(30,101.9),(27,103),(23.5,103)]),
 ('ETH_LEDG','15',( 'R31'),(0,0),[(32.96,101.5),(30,101.5),(27,100),(23.5,100)])]:
...
for name,pn,ref,via_xy,pts in [
    ('/ETHERNET/GBE_LED_Y_K','16','R30',(22,106),[(81.59,48.94),(79,48.94),(79,45),(10,45),(10,106)]),
    ('/ETHERNET/GBE_LED_G_K','18','R31',(22,97),[(70.87,48.94),(68,48.94),(68,40),(8,40),(8,97)])]:
    net=nets[name];last=xy(P(b,'J2',pn));z=xy(P(b,ref,'2'));via(b,net,via_xy)
    for q in pts[1:]:seg(b,net,last,q,pcbnew.B_Cu);last=q
