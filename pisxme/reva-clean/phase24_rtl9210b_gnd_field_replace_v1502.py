"""V1502: single authoritative In1 GND field after removing the small field."""
from pathlib import Path
import pcbnew
H=Path(__file__).resolve().parent; BASE=H/'PHASE24_RTL9210B_GND_REROUTE_RTL3V3_V1491.kicad_pcb'; OUT=H/'PHASE24_RTL9210B_GND_FIELD_REPLACE_V1502.kicad_pcb'; b=pcbnew.LoadBoard(str(BASE)); V=lambda x,y: pcbnew.VECTOR2I_MM(float(x),float(y)); g=b.FindNet('GND'); assert g
for q in list(b.GetTracks()):
 if q.GetNetname()=='GND' and q.GetLayer()==pcbnew.B_Cu: b.RemoveNative(q)
for z in list(b.Zones()):
 if z.GetLayer()==pcbnew.In1_Cu and z.GetNetname()=='GND': b.RemoveNative(z)
