"""V475: lane pairs with TX layer split before shared B.Cu corridors."""
from pathlib import Path
import pcbnew
H=Path(__file__).resolve().parent
src=H/'PHASE24_RTL9210B_ROTATED_SOURCE_PLANAR_V462.kicad_pcb'; out=H/'PHASE24_RTL9210B_LANE_PAIRS_LAYER_SPLIT_V475.kicad_pcb'
...
for name,q,mid,row,side,entry,dst in [('LANE0_TXN',(115,67.4),(120,67.4),(120,80),(142,80),(134.75,59.2),(135.25,62.725)),('LANE0_TXP',(116,66.5),(121,66.5),(121,79),(143,79),(135.75,57.5),(135.75,62.725))]:
 n=b.FindNet(name);p=u.FindPadByNumber({'LANE0_TXN':'67','LANE0_TXP':'68'}[name]);pp=p.GetPosition();a=(pcbnew.ToMM(pp.x),pcbnew.ToMM(pp.y));seg(b,n,F,a,(a[0],q[1]));seg(b,n,F,(a[0],q[1]),q);via(b,n,q);seg(b,n,B,q,mid);via(b,n,mid);seg(b,n,F,mid,row);seg(b,n,F,row,side);via(b,n,side);seg(b,n,B,side,(side[0],entry[1]));seg(b,n,B,(side[0],entry[1]),entry);via(b,n,entry);seg(b,n,F,entry,dst)
