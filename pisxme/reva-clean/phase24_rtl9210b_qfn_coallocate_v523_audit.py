"""V523 native RTL_3V3 connectivity and exact branch negative control."""
import pcbnew
from pathlib import Path
PCB=Path(__file__).resolve().parent/'PHASE24_RTL9210B_QFN_COALLOCATE_V523.kicad_pcb'
...
print('PASS V523 native U1.20/U1.34/U1.39 -> C3.1 RTL_3V3 connectivity')
...
for x in t.GetTracks():
 ...
 if isinstance(x,pcbnew.PCB_VIA):
  ...
  if xy in [(99.5,56.8),(99.5,60.4)]: branch.append(x)
 else:
  a=x.GetStart(); z=x.GetEnd(); aa=(round(a.x/1e6,4),round(a.y/1e6,4)); zz=(round(z.x/1e6,4),round(z.y/1e6,4))
  if (aa,zz) in [((102.8,58.05),(102.8,56.8)),((102.8,56.8),(99.5,56.8)),((99.5,56.8),(99.5,60.4)),((99.5,60.4),(102.05,60.4))]: branch.append(x)
assert branch, 'new U1.34 branch not found'
for x in branch:t.RemoveNative(x)
q=pads(t); assert not reach(t,q['U1','34'],q['U1','20'])
print('PASS V523 exact U1.34 branch negative control')
