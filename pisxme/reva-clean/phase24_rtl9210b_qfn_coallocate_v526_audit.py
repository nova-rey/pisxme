"""V526 native U1.39 rail connectivity and exact branch negative control."""
import pcbnew
from pathlib import Path
PCB=Path(__file__).resolve().parent/'PHASE24_RTL9210B_QFN_COALLOCATE_V526.kicad_pcb'
...
print('PASS V526 native U1.20/U1.34/U1.39 -> C3.1 RTL_3V3 connectivity')
...
for x in t.GetTracks():
 ...
 if (aa,zz) in [((100.5,60.4),(100.5,69.0)),((100.5,69.0),(112.5,69.0)),((112.5,69.0),(112.5,57.2)),((112.5,57.2),(109.5,57.2))]: branch.append(x)
assert branch, 'new U1.39 branch not found'
for x in branch:t.RemoveNative(x)
q=pads(t); assert not reach(t,q['U1','39'],q['U1','20'])
print('PASS V526 exact U1.39 branch negative control')
