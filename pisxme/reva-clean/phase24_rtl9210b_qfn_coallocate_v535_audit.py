"""V535 native four-source 3V3 connectivity and U1.52 negative control."""
import pcbnew
from pathlib import Path
PCB=Path(__file__).resolve().parent/'PHASE24_RTL9210B_QFN_COALLOCATE_V535.kicad_pcb'
...
for x in [('U1','20'),('U1','34'),('U1','39'),('U1','52'),('C3','1')]: assert reach(b,q['U1','20'],q[x]), x
print('PASS V535 native four-source RTL_3V3 connectivity')
...
for x in t.GetTracks():
 ...
 if isinstance(x,pcbnew.PCB_VIA):
  ...
  if xy==(101.5,68.5): branch.append(x)
 else:
  a=x.GetStart();z=x.GetEnd(); aa=(round(a.x/1e6,4),round(a.y/1e6,4));zz=(round(z.x/1e6,4),round(z.y/1e6,4))
  if (aa,zz) in [((101.5,67.5),(101.5,68.5)),((101.5,68.5),(103.5,68.5)),((103.5,68.5),(103.5,65.5))]: branch.append(x)
assert branch
for x in branch:t.RemoveNative(x)
q=pads(t); assert not reach(t,q['U1','52'],q['U1','20'])
print('PASS V535 exact U1.52 branch negative control')
