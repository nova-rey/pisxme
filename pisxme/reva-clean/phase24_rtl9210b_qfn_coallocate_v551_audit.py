"""V551 native bottom 1V1 connectivity and exact branch negative control."""
import pcbnew
from pathlib import Path
PCB=Path(__file__).resolve().parent/'PHASE24_RTL9210B_QFN_COALLOCATE_V551.kicad_pcb'
...
print('PASS V551 native U1.55/U1.60/U1.63 -> C4.1 RTL_1V1 connectivity')
...
for x in t.GetTracks():
 ...
 if isinstance(x,pcbnew.PCB_VIA):
  ...
  if xy in [(103.5,72.5),(105.5,72.5),(108.0,72.5)]:branch.append(x)
 else:
  a=x.GetStart();z=x.GetEnd();aa=(round(a.x/1e6,4),round(a.y/1e6,4));zz=(round(z.x/1e6,4),round(z.y/1e6,4))
  if aa[1]>=72.5 and zz[1]>=72.5:branch.append(x)
assert branch
for x in branch:t.RemoveNative(x)
q=pads(t);assert not reach(t,q['U1','60'],q['C4','1']);print('PASS V551 exact bottom-edge branch negative control')
