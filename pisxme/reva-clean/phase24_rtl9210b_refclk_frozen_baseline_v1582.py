"""V1582: separated west-edge REFCLK transitions on the frozen U1 baseline."""
from pathlib import Path
import pcbnew

H = Path(__file__).resolve().parent
base = H / "PHASE24_RTL9210B_U155_REHOME_V1517.kicad_pcb"
out = H / "PHASE24_RTL9210B_REFCLK_FROZEN_BASELINE_V1582.kicad_pcb"
...
for name, source, transition, yb, xj in (
    ("REFCLK_P", (94.05,70.4), (93.5,70.4), 56.0, 137.25),
    ("REFCLK_N", (94.05,70.8), (92.5,70.8), 59.0, 136.75),
):
    n = b.FindNet(name); assert n
    tr(n, source, transition, F)
    via(n, *transition)
    tr(n, transition, (transition[0], yb), B)
    tr(n, (transition[0], yb), (106.0, yb), B)
    via(n, 106.0, yb)
    tr(n, (106.0, yb), (xj, yb), F)
