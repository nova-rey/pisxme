"""V177: B.Cu source handoffs around translated XTAL_IN/XTAL_OUT spans."""
from pathlib import Path
import pcbnew
HERE=Path(__file__).resolve().parent; SRC=HERE/'PHASE24_RTL9210B_LOWER_1V1_HANDOFF_V160.kicad_pcb'; BASE=HERE/'PHASE24_RTL9210B_REFCLK_SUPPORT_RELOCATED_V174.kicad_pcb'; OUT=HERE/'PHASE24_RTL9210B_REFCLK_SUPPORT_RELOCATED_V177.kicad_pcb'; F,B=pcbnew.F_Cu,pcbnew.B_Cu
W=.13208
...
def copy_in(dst,src,n):
 for x in src.GetTracks():
  ...
  if not all(90<=p.x/1e6<=115 and 70<=p.y/1e6<=82 for p in pts):continue
...
def main():
 b=pcbnew.LoadBoard(str(BASE));s=pcbnew.LoadBoard(str(SRC));copy_in(b,s,b.FindNet('XTAL_IN'))
 ...
 tr(b,p,F,(98.4,65.95),(98.2,66.8));via(b,p,(98.2,66.8));tr(b,p,B,(98.2,66.8),(93.,66.8));tr(b,p,B,(93.,66.8),(93.,60.));tr(b,p,B,(93.,60.),(138.5,60.));via(b,p,(138.5,60.));tr(b,p,F,(138.5,60.),(137.25,62.725))
 tr(b,n,F,(98.8,65.95),(99.,66.8));via(b,n,(99.,66.8));tr(b,n,B,(99.,66.8),(106.5,66.8));tr(b,n,B,(106.5,66.8),(106.5,60.8));tr(b,n,B,(106.5,60.8),(136.75,60.8));via(b,n,(136.75,60.8));tr(b,n,F,(136.75,60.8),(136.75,62.725))
