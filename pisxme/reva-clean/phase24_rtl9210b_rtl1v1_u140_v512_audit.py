...
def ok(b):
 b.BuildConnectivity(); q=pads(b); conn=b.GetConnectivity(); seen={id(q[G[0]])}; todo=[q[G[0]]]
 while todo:
  for item in conn.GetConnectedItems(todo.pop()):
   if id(item) not in seen: seen.add(id(item)); todo.append(item)
 return all(id(q[x]) in seen for x in G[1:])
