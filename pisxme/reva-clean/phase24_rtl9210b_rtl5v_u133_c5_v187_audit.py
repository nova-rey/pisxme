"""Saved native connectivity audit and negative control for V187."""
from pathlib import Path
import tempfile
import pcbnew

HERE = Path(__file__).resolve().parent
PCB = HERE / "PHASE24_RTL9210B_RTL5V_U133_C5_V187.kicad_pcb"
EXPECTED = {("U1", "33"), ("C5", "1")}
...
def main():
    ...
    assert EXPECTED <= pads(board, "U1", "33"), pads(board, "U1", "33")
    bad = pcbnew.LoadBoard(str(PCB))
    victim = next(x for x in bad.GetTracks()
                  if type(x).__name__ == "PCB_TRACK" and x.GetNetname() == "RTL_5V")
    ...
    with tempfile.NamedTemporaryFile(suffix=".kicad_pcb") as handle:
        ...
        assert not EXPECTED <= pads(broken, "U1", "33")
    print("V187 native U1.33/C5.1 connectivity PASS; trace-removal negative control PASS")
