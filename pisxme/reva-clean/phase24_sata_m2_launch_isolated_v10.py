#!/usr/bin/env python3
"""V10: V9 with the RXP source transition moved below RXN's escape."""
from pathlib import Path
import pcbnew
ROOT=Path(__file__).resolve().parent
BASE=ROOT/'PHASE24_PATHA_M2_LANE0_LAUNCH_ISOLATED_V1.kicad_pcb'
OUT=ROOT/'PHASE24_PATHA_M2_LANE0_LAUNCH_ISOLATED_V10.kicad_pcb'
...
def main():
    ...
    specs=[
      ('M2_SATA_A_P_PCIE_TXP0','2','49',(220,153.5),'F',[(175,147.2),(175,132),(190,132),(208,132),(220,153.5)]),
      ('M2_SATA_A_N_PCIE_TXN0','3','47',(218,153.5),'B',[(173.5,147.6),(173.5,136),(192,136),(210,136),(218,153.5)]),
      ('M2_SATA_B_P_PCIE_RXN0','6','41',(214,153.5),'F',[(169,148.8),(169,154),(190,154),(206,154),(214,153.5)]),
      ('M2_SATA_B_N_PCIE_RXP0','7','43',(216,153.5),'B',[(171.5,151),(171.5,165),(206,165),(206,168),(213,168),(216,153.5)]),
    ]
    for name,up,jp,target,layer,pts in specs:
        ...
        if layer=='F': path(b,n,[src,(sx,sy)]+pts[1:],F)
            ...
            if name.endswith('RXP0'):
                path(b,n,[src,(171.5,149.2),(sx,sy)],F)
            else:
                path(b,n,[src,(sx,sy)],F)
            via(b,n,(sx,sy)); path(b,n,pts[1:],B)
        via(b,n,target); path(b,n,[dst,(dst[0],157),target],F)
