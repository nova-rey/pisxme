...
import pcbnew, math
R=Path(__file__).resolve().parent; BASE=R/'PHASE24_STORAGE_VCCK_LOCAL_V1.kicad_pcb'; OUT=R/'PHASE24_STORAGE_SATA_ASTAR_V1.kicad_pcb'; F,B=pcbnew.F_Cu,pcbnew.B_Cu; STEP=.25
...
def occ(b):
 ...
 return o
def route(o,a,z):
 s=(*g(a),F); t=(*g(z),F); l={F:set(o[F]),B:set(o[B])}
 for x,y,lay in ((s[0],s[1],F),(t[0],t[1],F)):
  for dx in range(-8,9):
...
for t in list(b.GetTracks()):
 if any(x in t.GetNetname() for x in ('BRIDGE_SATA_','TUSB_SATA_')): b.RemoveNative(t)
