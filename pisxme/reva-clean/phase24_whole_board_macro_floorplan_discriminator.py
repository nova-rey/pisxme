...
lines += [
    ...
    "",
    "## Functional-neighborhood interpretation",
    "",
    "- Ethernet: the native J7 west launch is adjacent to the SWAP Ethernet island; the current-to-SWAP source distance falls from 59.2 mm to 22.5 mm. The historical east/right placement forces the Ethernet corridor through storage and power acreage.",
    "- PCIe/V100: the native J7 PCIe launch is on the east/right group and the J1/SXM2 endpoint remains eastward. The review leaves this anchor unchanged because every tested migration increased source distance without reducing a competing corridor enough to compensate.",
    "- Storage: the complete USB3-to-U7-to-SATA-to-J3 neighborhood, including Y1/R23/C42/C43 and C16/C17/C19/C30-C33, remains coherent in SWAP. Its source distance falls from 65.9 mm to 49.5 mm; detailed copper is intentionally excluded from this topology comparison.",
    "- SERVICE: J4/U8 remains near the native USB2 launch (20.1 mm centroid distance, 8.5 mm nearest pad). It is not an effective swap target because moving it would create a new cross-board low-speed corridor.",
    "- Power input/protection: J5/J6/F1/F2/U1/U2/Q1/Q2 have no CM5 source-prefix group in the native PCB metric; they are evaluated by connector access, high-current entry direction, protection-block coherence, and corridor occupancy. The power-entry block is unchanged in every promoted topology probe.",
    "- Regulator/load delivery: U3/U4/U5 likewise have no single CM5 launch centroid; they are evaluated as coherent rail-delivery islands relative to their loads, return access, thermal acreage, and obstruction of Ethernet/storage corridors. No regulator topology or anchor was moved.",
    "- The coarse body-box screen is not a complete mechanical signoff: actual courtyards, 3-D bodies, mating access, mounting features, enclosure clearance, power-current geometry, and reference-plane continuity remain downstream checks after a candidate is routed.",
    "",
    "### Classification rule",
    "",
